...
from tools.series import collatz
            ...
            if event.user_type == pygame_gui.UI_BUTTON_PRESSED:
                if event.ui_element == self.calculate_button:
                    try:
                        ...
                        result = collatz(n)
                        self.result_surf = self.font.render(f"Collatz Steps({n}) = {result}", True, (205, 214, 244))
                    except ValueError as e:
                        self.error_message = f"Error: {e}"
    def draw(self, surface):
        surface.fill((30, 30, 46))
        title = self.title_font.render("Collatz Conjecture", True, (205, 214, 244))
